...
- **Windows 시작 시 자동 실행** — 헤더의 ⚡ 자동 시작 슬라이더 한 번에 켜고 끔 (레지스트리 Run 키 + `--minimized` 플래그). EXE 위치를 바꿔도 다음 실행 시 현재 경로로 자동 갱신
...
- **시스템 트레이** — 창 X 누르면 트레이로(감시 유지). 트레이 메뉴에서 감시 개수, Stop All, 자동 실행 토글, 종료
- **다크/라이트 테마 토글**
...
| Windows 켜질 때부터 동기화 | 자동 감시 시작 후 헤더의 "⚡ 자동 시작" 슬라이더 ON (또는 트레이 메뉴에서 체크) |
